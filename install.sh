...
cat >> /etc/hosts <<EOF
172.22.101.101 node1
172.22.101.102 node2
172.22.101.103 node3
EOF
...
FLANNEL_ETCD_ENDPOINTS="http://172.22.101.101:2379"
...
then
    ...
    echo https://172.22.101.101:`kubectl -n kube-system get svc kubernetes-dashboard -o=jsonpath='{.spec.ports[0].port}'`
